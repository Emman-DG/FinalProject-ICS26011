...
class Mv6 : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        movieName = findViewById(R.id.MovieName06)
        movieDesc = findViewById(R.id.MovieDesc06)
        addToWatchList = findViewById(R.id.addButton6)

        val movieId = "6" // ID of the second movie
        retrieveMovieDetails(movieId)

        //Trailer
        val videoView = findViewById<VideoView>(R.id.Mv6)
        val packageName = "android.resource://" + getPackageName() + "/" + R.raw.thedictator
            ...
            userMovieRef.addListenerForSingleValueEvent(object : ValueEventListener {
                override fun onDataChange(snapshot: DataSnapshot) {
                    if (snapshot.exists()) {
                        ...
                        Toast.makeText(this@Mv6, "Movie already added to watchlist", Toast.LENGTH_SHORT).show()
                    } else {
                        // Movie is not in the watchlist, add it
                        userMovieRef.setValue(true)
                        Toast.makeText(this@Mv6, "Added to Watchlist", Toast.LENGTH_SHORT).show()
                    }
                }

                override fun onCancelled(error: DatabaseError) {
                    // Handle error
                    Toast.makeText(this@Mv6, "Error checking watchlist", Toast.LENGTH_SHORT).show()
                }
            })
