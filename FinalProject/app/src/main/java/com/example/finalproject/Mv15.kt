...
class Mv15 : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_mv15)

        // Initialization
        database = FirebaseDatabase.getInstance()
        userMoviesReference = database.reference.child("user_movies")
        movieName = findViewById(R.id.MovieName15)
        movieDesc = findViewById(R.id.MovieDesc15)
        addToWatchList = findViewById(R.id.addButton15)
        youTubePlayerView = findViewById(R.id.youtubePlayerView)

        val movieId = "15" // ID of the second movie
            ...
            userMovieRef.addListenerForSingleValueEvent(object : ValueEventListener {
                override fun onDataChange(snapshot: DataSnapshot) {
                    if (snapshot.exists()) {
                        ...
                        Toast.makeText(this@Mv15, "Movie already added to watchlist", Toast.LENGTH_SHORT).show()
                    } else {
                        // Movie is not in the watchlist, add it
                        userMovieRef.setValue(true)
                        Toast.makeText(this@Mv15, "Added to Watchlist", Toast.LENGTH_SHORT).show()
                    }
                }

                override fun onCancelled(error: DatabaseError) {
                    // Handle error
                    Toast.makeText(this@Mv15, "Error checking watchlist", Toast.LENGTH_SHORT).show()
                }
            })
