...
class Mv13 : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_mv13)

        // Initialization
        database = FirebaseDatabase.getInstance()
        userMoviesReference = database.reference.child("user_movies")
        movieName = findViewById(R.id.MovieName13)
        movieDesc = findViewById(R.id.MovieDesc13)
        addToWatchList = findViewById(R.id.addButton13)
        youTubePlayerView = findViewById(R.id.youtubePlayerView)

        val movieId = "13" // ID of the second movie
            ...
            userMovieRef.addListenerForSingleValueEvent(object : ValueEventListener {
                override fun onDataChange(snapshot: DataSnapshot) {
                    if (snapshot.exists()) {
                        ...
                        Toast.makeText(this@Mv13, "Movie already added to watchlist", Toast.LENGTH_SHORT).show()
                    } else {
                        // Movie is not in the watchlist, add it
                        userMovieRef.setValue(true)
                        Toast.makeText(this@Mv13, "Added to Watchlist", Toast.LENGTH_SHORT).show()
                    }
                }

                override fun onCancelled(error: DatabaseError) {
                    // Handle error
                    Toast.makeText(this@Mv13, "Error checking watchlist", Toast.LENGTH_SHORT).show()
                }
            })
