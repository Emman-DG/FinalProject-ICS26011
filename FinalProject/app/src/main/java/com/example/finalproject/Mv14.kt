...
class Mv14 : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_mv14)
        ...
        addToWatchList = findViewById(R.id.addButton14)
        youTubePlayerView = findViewById(R.id.youtubePlayerView)

        val movieId = "14" // ID of the second movie
            ...
            userMovieRef.addListenerForSingleValueEvent(object : ValueEventListener {
                override fun onDataChange(snapshot: DataSnapshot) {
                    if (snapshot.exists()) {
                        ...
                        Toast.makeText(this@Mv14, "Movie already added to watchlist", Toast.LENGTH_SHORT).show()
                    } else {
                        // Movie is not in the watchlist, add it
                        userMovieRef.setValue(true)
                        Toast.makeText(this@Mv14, "Added to Watchlist", Toast.LENGTH_SHORT).show()
                    }
                }

                override fun onCancelled(error: DatabaseError) {
                    // Handle error
                    Toast.makeText(this@Mv14, "Error checking watchlist", Toast.LENGTH_SHORT).show()
                }
            })
