...
class Mv8 : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        movieName = findViewById(R.id.MovieName08)
        movieDesc = findViewById(R.id.MovieDesc08)
        addToWatchList = findViewById(R.id.addButton8)

        val movieId = "8" // ID of the second movie
        retrieveMovieDetails(movieId)

        //Trailer
        val videoView = findViewById<VideoView>(R.id.Mv8)
        val packageName = "android.resource://" + getPackageName() + "/" + R.raw.sausageparty
            ...
            userMovieRef.addListenerForSingleValueEvent(object : ValueEventListener {
                override fun onDataChange(snapshot: DataSnapshot) {
                    if (snapshot.exists()) {
                        ...
                        Toast.makeText(this@Mv8, "Movie already added to watchlist", Toast.LENGTH_SHORT).show()
                    } else {
                        // Movie is not in the watchlist, add it
                        userMovieRef.setValue(true)
                        Toast.makeText(this@Mv8, "Added to Watchlist", Toast.LENGTH_SHORT).show()
                    }
                }

                override fun onCancelled(error: DatabaseError) {
                    // Handle error
                    Toast.makeText(this@Mv8, "Error checking watchlist", Toast.LENGTH_SHORT).show()
                }
            })
