...
class Mv3 : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        movieName = findViewById(R.id.MovieName03)
        movieDesc = findViewById(R.id.MovieDesc03)
        addToWatchList = findViewById(R.id.addButton3)
        youTubePlayerView = findViewById(R.id.youtubePlayerView3)

        val movieId = "3" // ID of the second movie
            ...
            userMovieRef.addListenerForSingleValueEvent(object : ValueEventListener {
                override fun onDataChange(snapshot: DataSnapshot) {
                    if (snapshot.exists()) {
                        ...
                        Toast.makeText(this@Mv3, "Movie already added to watchlist", Toast.LENGTH_SHORT).show()
                    } else {
                        // Movie is not in the watchlist, add it
                        userMovieRef.setValue(true)
                        Toast.makeText(this@Mv3, "Added to Watchlist", Toast.LENGTH_SHORT).show()
                    }
                }

                override fun onCancelled(error: DatabaseError) {
                    // Handle error
                    Toast.makeText(this@Mv3, "Error checking watchlist", Toast.LENGTH_SHORT).show()
                }
            })
